        ...
        const fetchData = async () => {
            try {
                ...
                const addressResponse = await axios.get(
                    'http://localhost:5000/api/v2/user/addresses',
                    {
                        params: { email: email },
                    }
                );
                ...
                const cartResponse = await axios.get(
                    'http://localhost:5000/api/v2/product/cartproducts',
                    {
                        params: { email: email },
                    }
                );
    ...
    const handlePlaceOrder = async (paymentType = 'cod', paypalOrderData = null) => {
        try {
            ...
            const response = await axios.post(
                'http://localhost:5000/api/v2/orders/place-order',
                payload
            );
                        ...
                        {paymentMethod === 'paypal' && (
                            <div className='mt-4' style={{ maxWidth: '500px' }}>
                                <PayPalScriptProvider
                                    options={{
                                        'client-id': "AXzFw3V_cKQ4qCzpfoL8nu1LLYyVEtTVf8TMkG61TcaTWtIJeADkqF_H6pNxE9gDf1g6_XtSE4kYkqsU",
                                    }}
                                >
                                    <PayPalButtons
